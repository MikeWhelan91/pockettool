...
function renderWordOps(ops: Op[]) {
  return (
    <div
      className="border border-neutral-800 rounded-lg p-3 bg-transparent text-sm leading-6 overflow-auto font-mono whitespace-pre-wrap"
    >
      {ops.map((op, i) => {
        if (op.type === 'eq') {
...
function renderLineOps(ops: Op[]) {
  return (
    <pre className="border border-neutral-800 rounded-lg p-3 bg-transparent text-sm overflow-auto font-mono whitespace-pre-wrap">
      {ops
        .map((op) => {
...
export default function DiffClient() {
  ...
  const [view, setView] = useState<View>("words");
   const hasInput = a.trim().length > 0 || b.trim().length > 0;
  ...
  function copyHTML() {
    ...
    if (!el) return;
    navigator.clipboard?.writeText(el.innerHTML).catch(() => {});
  }
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <div className="flex items-end gap-2">
            {view === "words" ? (
              ...
              <button className="btn-ghost" onClick={copyUnified}>
                Copy unified text
              </button>
            )}
          </div>
...
{/* Output */}
{hasInput && (
  <div id="diff-render-root" className="mt-3">
