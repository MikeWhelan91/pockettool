...
export default function HomePage() {
  return (
    <div className="grid gap-10">
      ...
      <section
        className="relative overflow-hidden rounded-[var(--radius)] border border-line bg-[hsl(var(--card))] shadow-[0_10px_40px_-10px_hsl(var(--ring)/.25)]"
      >
        {/* top highlight */}
  ...
  >
    utilixy.com
            ...
            <p className="mx-auto mt-2 text-[1.05rem] text-muted max-w-[70ch]">
              Reorder, rotate, merge, split, add page numbers, headers/footers, watermarks, extract text,
              convert images↔PDF, stamp QR, redact, and optimize — <b>all in your browser</b>.
            </p>

            {/* CTAs */}
            <div className="mt-4 flex flex-wrap items-center justify-center gap-3">
              <Link href="/pdf" className="btn">Open PDF Studio</Link>
              <a href="#tools" className="btn-ghost">Browse all tools</a>
              <span className="text-xs text-muted">Private · Nothing uploaded</span>
            </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              { t: "Assemble & organize", d: "Reorder, rotate, delete pages. Batch merge docs." },
              { t: "Annotate & label", d: "Page numbers, headers/footers, watermarks, QR stamps." },
              { t: "Convert & extract", d: "Images→PDF, PDF→PNG/ZIP, extract text, quick redact, split." },
            ].map((f, i) => (
              <div key={i} className="rounded-xl border border-line bg-[hsl(var(--bg))]/50 p-4">
          ...
          {tools.map((t) => (
            <Link key={t.href} href={t.href} className="no-underline">
              <article
                className={
                  "tile p-5 h-full " +
                  (t.href === "/pdf" ? "relative border-2 border-[hsl(var(--ring))] lg:col-span-2" : "")
                }
              >
                {t.href === "/pdf" && (
